...
function wscs_theme_styles() { 
	...
	wp_enqueue_style('googlefont_css', 'https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet'); 
	//array() and false are default values; needed to fill last parameter with media query
	wp_enqueue_style('materialize_css', get_template_directory_uri() . '/css/small.css', array(), false, '(max-width:992px)');  
	wp_enqueue_style('materialize_css', get_template_directory_uri() . '/css/large.css', array(), false, '(min-width:993px)');  
}
